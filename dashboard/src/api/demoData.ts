// Deterministic in-memory demo data generator (client-side port of
// src/server/demo.ts). Never touches any backend or database — used only when
// the dashboard is built with VITE_DEMO=1 (see api/client.ts).
//
// Keep in sync with src/server/demo.ts: same seed and generation order produce
// the identical 36 sessions the CLI `agentlens dashboard --demo` renders.
